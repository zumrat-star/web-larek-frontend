...
**Методы:**
*   `constructor(container: HTMLElement, events: IEvents)` - инициализация модального окна
...
#### Basket
**Назначение:** Управляет отображением состояния корзины в шапке сайта. Рендерит себя на основе `template#basket`.
**Поля:**
...
**Методы:**
*   `renderList(items: ICard[]): void` - очищает список и отрисовывает в нем переданные товары, используя компонент `BasketItem` и `template#card-basket`.
...
*   `Card` - товары в каталоге
*   `CardPreview` - просмотр товара
*   `Basket` - корзина и счетчик
*   `BasketItem` - позиция в корзине
*   `OrderForm` - форма заказа
*   `ContactsForm` - форма контактов
...
Презентер вызывает basket.render() с новыми данными
Компонент Basket обновляет отображение
...
После успешного ответа сервера открывается модальное окно Success
